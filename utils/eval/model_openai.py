...
class OpenAI_Model:
    def __init__(self):
        if "AZURE_OPENAI_API_KEY" in os.environ and "AZURE_OPENAI_ENDPOINT" in os.environ:
            ...
            assert "OPENAI_API_KEY" in os.environ, (
                ...
                "Pass --api-key YOUR_KEY when running eval."
            )
    ...
    def generate(self, messages, model="gpt-4o-mini", timeout=180, max_retries=10, temperature=1.0, is_json=False, return_metadata=False, max_tokens=None, variables={}):
        kwargs = {}
        if is_json:
        ...
        while True:
            try:
                ...
                print(f"Error: {e}， retry {N}")
                N += 1
                if N >= max_retries:
                    raise Exception("Failed to get response")
                else:
                    time.sleep(random.randint(1 * N, 20 * N))
...
def generate(messages, model="gpt-4o-mini", **kwargs):
    return _get_model().generate(messages, model=model, **kwargs)

def generate_json(messages, model="gpt-4o-mini", **kwargs):
    return _get_model().generate_json(messages, model=model, **kwargs)
