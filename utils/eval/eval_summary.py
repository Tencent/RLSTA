...
def evaluate_insights(insights, summary, evaluator_model_card):
...
def compute_single_sample_scores(summary, evals, insightid2ref_citations, partial_score=0.5, cite_offset=0): 
    bullets_obj = summary2bullets(summary)
    bullets = bullets_obj["bullets"]
    ...
    for e in evals:
        ...
        # Citation scores are only calculated if there is coverage
...
def compute_single_sample_results(summary, evals, insightid2ref_citations, partial_score=0.5, cite_offset=0):
    ...
    for k, v in scores.items():
        # 'trim_ratio' is a float, not a list, so we pass it through directly
        if not isinstance(v, list):
            results[k] = v
            continue
            
        # FIX: Check if list is empty before calculating mean to avoid NaN
        if len(v) == 0:
            results[k] = 0.0
        else:
