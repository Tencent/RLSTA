...
from .eval_code import check_correctness


class TaskCode(Task):
    def __init__(self):
        # self.fully_specified_prompt_lcb = ""
        ...
        self.system_prompt_lcb = """You are an expert Python programmer. You will be given a question (problem specification) and will generate a correct Python program that matches the specification and passes all tests.
...
[Important] I will provide additional conditions incrementally in turns rather than all at once. Do not provide a concrete answer attempt you have all necessary details.
...
Update your solution accordingly while maintaining correctness under all conditions provided so far."""
        self.system_prompt_humaneval = """You are helping a user write a Python function to solve a programming question. If something is not clear, you can ask the user to clarify what they need."""
        self.fully_specified_prompt_humaneval = """You must generate a valid Python function that satisfies the following instruction:
...
- [Validity] The Python function must be valid and executable. Do not include any other text or comments. (```python```)"""
        self.fully_specified_prompt_lcb = """### Question:
[[QUESTION]]

### Format: [[FORMATTING_PREAMBLE]]
[[FORMATTING]]
...
### Answer: (use the provided format with backticks)"""
    ...
    def get_dataset_file(self) -> str:
        return os.path.join(os.path.dirname(__file__), "../../sharded_data/lost_in_conversation_code.json")
    ...
    def extract_answer(self, text: str) -> str:
        ...
        if "class Solution" in text:
            ...
            if len([c for c in code_blocks if "class Solution" in c]) > 0:
